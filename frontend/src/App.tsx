...
/**
 * Builds a deterministic photo-to-variant assignment so that:
 *   1. Each photo ends up in AT MOST ONE variant (no duplicates across variants).
 *   2. Photos sharing the same detected `colorGroup` are grouped into the same
 *      variant whenever possible.
 *   3. Orphan photos (those that don't strictly match any variant via
 *      `isColorMatchFrontend`) are rescued by assigning them to the variant
 *      whose color tag dominates their group; if no group consensus is found,
 *      they fall back to the most-similar variant or are left unassigned.
 *
 * Returns a Record keyed by variant name → list of image IDs.
 */
export function buildAutoVariantAssignments(
  variants: { name: string }[],
  images: Pick<UploadedImage, "id" | "colorGroup" | "fileName">[]
): Record<string, string[]> {
  const result: Record<string, string[]> = {};
  variants.forEach(v => { result[v.name] = []; });
  if (variants.length === 0 || images.length === 0) return result;

  const variantColors = variants.map(v => parseVariantStringFrontend(v.name).color);
  const assignedImageIds = new Set<string>();

  // --- Pass 1: strict match — each image goes to the FIRST variant whose color matches it.
  for (const img of images) {
    for (let i = 0; i < variants.length; i++) {
      const vc = variantColors[i];
      if (!vc) continue;
      if (isColorMatchFrontend(vc, img.colorGroup || "other", img.fileName || "")) {
        result[variants[i].name].push(img.id);
  ...
  // --- Pass 2: orphan rescue by colorGroup consensus.
  // For each detected colorGroup that still has orphan photos, find the variant
  // that already received the MOST photos of that group and dump the orphans there.
  const orphansByGroup = new Map<string, string[]>();
  for (const img of images) {
    if (assignedImageIds.has(img.id)) continue;
    const grp = img.colorGroup || "other";
    if (!orphansByGroup.has(grp)) orphansByGroup.set(grp, []);
    orphansByGroup.get(grp)!.push(img.id);
  }

  for (const [grp, orphanIds] of orphansByGroup.entries()) {
    // Which variant already owns the most photos of this colorGroup?
    let bestVariantIdx = -1;
    let bestCount = 0;
    for (let i = 0; i < variants.length; i++) {
      const ownedIds = result[variants[i].name];
      const ownedOfThisGroup = ownedIds.filter(id => {
        const img = images.find(im => im.id === id);
        return img && (img.colorGroup || "other") === grp;
      }).length;
      if (ownedOfThisGroup > bestCount) {
        bestCount = ownedOfThisGroup;
        bestVariantIdx = i;
      }
    }

    // If at least one variant already holds photos of this group, route orphans there.
    if (bestVariantIdx >= 0) {
      result[variants[bestVariantIdx].name].push(...orphanIds);
      orphanIds.forEach(id => assignedImageIds.add(id));
      continue;
    }

    // No variant owns this group yet → assign all orphans of this group to the
    // variant with the fewest current photos (load-balance), preferring the first
    // variant that has zero photos.
    let leastIdx = 0;
    let leastCount = Infinity;
    for (let i = 0; i < variants.length; i++) {
      const cnt = result[variants[i].name].length;
      if (cnt < leastCount) {
        leastCount = cnt;
        leastIdx = i;
      }
    }
    result[variants[leastIdx].name].push(...orphanIds);
    orphanIds.forEach(id => assignedImageIds.add(id));
  }
...
export default function App() {
  ...
  const analyzeImage = async () => {
    ...
    try {
      ...
      if (response.ok && data.success && data.analysis) {
        ...
        const aiVariants = (analysis.variants || []).map((v: string) => ({ name: v, quantity: 0 }));
        const variantAssignments = buildAutoVariantAssignments(aiVariants, uploadedImages);
        setSuggestedVariants(aiVariants.map(v => ({
          name: v.name,
          quantity: 0,
          selectedImageIds: variantAssignments[v.name] || []
        })));
  ...
  // Reset all editable product state so the user can start a new product.
  // Keeps: psConfig, psCategories, history, language, VAT rate, default category selection.
  const handleStartNewProduct = (): void => {
    // Image gallery
    setUploadedImages([]);
    ...
    setUserHint("");
    setIsAnalyzing(false);
    setIsGeneratingField({ title: false, description_short: false, description: false });
    ...
    // Categories: reset to "Wszystkie produkty" (id 2) only; defaultCategoryId effect will resync
    ...
    // Scroll to top for a fresh-form feel
                    ...
                    <div className="flex items-start space-x-2">
                      ...
                      <div className="flex-1">
                        ...
                        <button
                          ...
                          data-testid="start-new-product-btn"
                          className="mt-3 w-full py-2.5 px-4 rounded-lg bg-[#008060] hover:bg-[#006e52] active:bg-[#005e46] text-white text-xs font-bold flex items-center justify-center gap-2 shadow-sm transition-all"
                        >
                          <Plus className="w-3.5 h-3.5" />
                          <span>Очистить и добавить новый товар</span>
                        </button>
                      </div>
                    </div>
                  ) : (
